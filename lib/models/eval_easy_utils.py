...
def eval_attributes(loader, model, split, opt):
  ...
  while True:
    ...
    _, att_scores = model.sub_encoder.extract_subj_feats(Feats['pool5'], Feats['fc7'])
...
def eval_split(loader, model, crit, split, opt):
  ...
  while True:
    ...
    for i, sent_id in enumerate(sent_ids):
      ...
      tic = time.time()
      scores_orig, sub_grid_attn, sub_attn, loc_attn, rel_attn, rel_ixs, weights, att_scores, att_loss = \
              model(Feats['pool5'], Feats['fc7'], Feats['lfeats'], Feats['dif_lfeats'], 
              Feats['cxt_fc7'], Feats['cxt_lfeats'],
              expanded_labels, img_pool5=Feats['img_pool5'])
 
      scores_orig = scores_orig.data.cpu().numpy()
      scores = scores_orig
	...
	entry['sub_grid_attn_erase'] = sub_grid_attn_erase[pred_ix].data.cpu().numpy().tolist() # list of 49 attn
        ...
        entry['weights_erase'] = weights_erase[pred_ix].data.cpu().numpy().tolist()   # list of 2 weights
