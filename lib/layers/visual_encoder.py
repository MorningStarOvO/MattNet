...
class SubjectEncoder(nn.Module):
  ...
  def extract_visual_attn(self, pool5, fc7, phrase_emb):
    batch, grids = pool5.size(0), pool5.size(2)*pool5.size(3)
...
"""
class RelationEncoderAtt(nn.Module):
  def __init__(self, opt):
    super(RelationEncoderAtt, self).__init__()
  ...
  def forward(self, cxt_feats, cxt_lfeats, phrase_emb):
    ...
    att = self.attn_fuse(torch.cat([rel_feats, phrase_emb.unsqueeze(1).expand(-1, num_cxt, -1)], 2)).squeeze(2) # (batch, num_cxt)
    att = F.softmax(att, dim=1)
    _, ix = torch.max(att, 1)
    attended_feats = torch.bmm(att.unsqueeze(1), rel_feats).squeeze(1)
    return attended_feats, ix

  def extract_rel_attn(self, cxt_feats, cxt_lfeats, phrase_emb):
    masks = (cxt_lfeats.sum(2) != 0).float()  # (n, num_cxt)
    phrase_emb = self.phrase_normalizer(phrase_emb)
...
"""
class LocationEncoderAtt(nn.Module):
  def __init__(self, opt):
    super(LocationEncoderAtt, self).__init__()
    init_norm = opt.get('visual_init_norm', 20)
    self.jemb_dim = opt['jemb_dim']
    self.lfeat_normalizer = Normalize_Scale(5, init_norm)
    self.dif_lfeat_normalizer = Normalize_Scale(5, init_norm)
    self.phrase_normalizer = Normalize_Scale(opt['jemb_dim'], init_norm)
    self.fc_lfeats = nn.Sequential(nn.Linear(5, opt['jemb_dim']), nn.ReLU())
    self.fc_diff = nn.Sequential(nn.Linear(5, opt['jemb_dim']), nn.ReLU())
    self.attn_subj = nn.Sequential(nn.Linear(opt['jemb_dim']*2, opt['jemb_dim']), nn.Tanh(), nn.Linear(opt['jemb_dim'], 1))
    self.attn_obj = nn.Sequential(nn.Linear(opt['jemb_dim']*2, opt['jemb_dim']), nn.Tanh(), nn.Linear(opt['jemb_dim'], 1))

  def forward(self, lfeats, dif_lfeats, phrase_emb):
    batch = lfeats.size(0)
    phrase_emb = self.phrase_normalizer(phrase_emb)
    dif_lfeats = dif_lfeats.view(batch*5, 5)
    dif_lfeats = self.fc_diff(self.dif_lfeat_normalizer(dif_lfeats)).view(batch, 5, self.jemb_dim)
    lfeats = self.fc_lfeats(self.lfeat_normalizer(lfeats)).view(batch, 1, self.jemb_dim)
 
    lfeats_att = self.attn_subj(torch.cat([lfeats, phrase_emb.unsqueeze(1)], 2))
    dif_lfeats_att = self.attn_obj(torch.cat([dif_lfeats, phrase_emb.unsqueeze(1).expand(-1, 5, -1)], 2))
    all_att = torch.cat([lfeats_att, dif_lfeats_att], 1).squeeze(2)
    all_lfeats = torch.cat([lfeats, dif_lfeats], 1) # n, 6, 512
    all_att = F.softmax(all_att, dim=1) # n, 6
    output = torch.bmm(all_att.unsqueeze(1), all_lfeats).squeeze(1)
    return output

  def extract_loc_attn(self, lfeats, dif_lfeats, phrase_emb):
    batch = lfeats.size(0)
    phrase_emb = self.phrase_normalizer(phrase_emb)
    dif_lfeats = dif_lfeats.view(batch*5, 5)
    dif_lfeats = self.fc_diff(self.dif_lfeat_normalizer(dif_lfeats)).view(batch, 5, self.jemb_dim)
