...
this_dir = "/home/data/yjgroup/qxy/qxy/CM-Att-Erase/lib/mrcn"
...
class Inference:
  ...
  def __init__(self, args):
    ...
    print("inference_no_imdb.py 使用如下配置: ")
    print('Using config:')
    pprint.pprint(cfg)
    # print("哈哈哈")
  ...
  def img_to_spatial_fc7(self, net_conv):
    ...
    scaled_boxes = Variable(torch.Tensor([0, 0, 16*(net_conv.shape[3]-1), 16*(net_conv.shape[2]-1)]).cuda()).expand(net_conv.shape[0], 4)
    rois = torch.cat([batch_inds, scaled_boxes], 1)
