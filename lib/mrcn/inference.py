...
class Inference:
  ...
  def load_net(self):
    ...
    #model = '/home/xhliu/refexp/REG/resnet101.pth'
  ...
  def extract_head_from_cropped(self, im):
    # extract head (1, 1024, im_height*scale/16.0, im_width*scale/16.0) in Variable cuda float
    # and im_info [[ih, iw, scale]] in float32 ndarray
    blobs, im_scales = self._get_blobs(im) 
    head_feat = self.net.extract_head(blobs['data'])
  ...
  def img_to_spatial_fc7(self, net_conv):
    """
    Arguments:
      net_conv (Variable)  : (1, 1024, H, W)
      im_info (float32) : [[ih, iw, scale]]
    ...
    # from net_conv to pool5 (bilinear pooling)
    x_indices = np.arange(net_conv.shape[2])
    y_indices = np.arange(net_conv.shape[3])
    
    batch_inds = Variable(net_conv.data.new(net_conv.shape[0], 1).zero_())
    pool5 = F.grid_sample(net_conv, grid)
